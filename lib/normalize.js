...
var _normalize_repeat_property = function (_field) {
  ...
  if (exports.is_script_directive(repeat)) {
    return _field;
  }
...
var _normalize_range_property = function (_field) {
  ...
  if (exports.is_script_directive(range)) {
    return _field;
  }
...
var _normalize_length_property = function (_field) {
  ...
  if (exports.is_script_directive(length)) {
    return _field;
  }
...
var _normalize_required_property = function (_field) {
  ...
  if (exports.is_script_directive(required)) {
    return _field;
  }
 ...
 *   Return true if the value provided in `_value` is a valid script
 *   directive. A script directive is an object containing a `javascript`
 *   property, whose value contains code to generate the intended
 *   property value. Some field properties (e.g. `validations` and
 *   `conditions`) support a superset of this and are implemented
 *   separately.
 */
exports.is_script_directive = function (_value) {

  if (!util.is_plain_object(_value)) {
    return false;
  }

  var keys = _.keys(_value);

  if (keys.length !== 1) {
    return false;
  }

  var javascript = _value.javascript;

  if (!_.isString(javascript) && !util.is_plain_object(javascript)) {
    return false;
  }
 ...
 */
exports.normalize_repeat_property = function (_field) {
 ...
 *   Within any field, a script directive may be provided in place of a
 *   static `required` property; the script directive will be executed if
 *   necessary, and its return value will ultimately be used to determine
 *   whether or not input is required. That returned value, however, may
 *   need to be validated and normalized before use; neither procedure will
 *   happen automatically because the JSON schema validation and
 *   normalization steps will have already taken place.  This function
 *   provides a normalization procedure for `required` specifiers that are
 *   returned from these script directives.
 */
exports.normalize_required_property = function (_field) {
 ...
 *   necessary, and its return value will ultimately be used as a default
 *   value if one is needed. That returned value, however, may need to be
 *   validated and normalized before use; neither procedure will happen
 *   automatically because the JSON schema validation and normalization
 *   steps will have already taken place. This function provides a
 *   normalization procedure for `default` specifiers that are returned
 *   from these script directives.
 */
exports.normalize_default_property = function (_field) {
 ...
 *   necessary, and its return value will ultimately be used as a length
 *   limitation. That returned value, however, may need to be validated and
 *   normalized before use; neither procedure will happen automatically
 *   because the JSON schema validation and normalization steps will have
 *   already taken place. This function provides a normalization procedure
 *   for `length` specifiers that are returned from these script directives.
 */
exports.normalize_length_property = function (_field) {
 ...
 *   necessary, and its return value will ultimately be used as a range
 *   restriction. That returned value, however, may need to be validated and
 *   normalized before use; neither procedure will happen automatically
 *   because the JSON schema validation and normalization steps will have
 *   already taken place. This function provides a normalization procedure
 *   for `range` specifiers that are returned from these script directives.
 */
exports.normalize_range_property = function (_field) {
