 ...
 */
var _normalize_validations_property = function (_field, _default_language) {
  if (util.is_omitted_value(_field.validations)) {
    _field.validations = {};
  }
  if (util.is_omitted_value(_field.validations.operator)) {
    _field.validations.operator = "&&";
  }
  if (util.is_omitted_value(_field.validations.javascript)) {
    _field.validations.javascript = {};
  } else if (_.isString(_field.validations.javascript)) {
    _field.validations.javascript = {
      "default": _field.validations.javascript
    };
  }
  if (util.is_omitted_value(_field.validations.registered)) {
    _field.validations.registered = {};
  } else if (_.isString(_field.validations.registered)) {
    var name = _field.validations.registered;
    _field.validations.registered = {};
    _field.validations.registered[name] = true;
  }
  return _field;
};
 ...
 *   to an object, and the registered property exists and is an object.
 */
var _normalize_conditions_property = function (_field, _default_language) {
  if (util.is_omitted_value(_field.conditions)) {
    _field.conditions = {};
  }
  if (util.is_omitted_value(_field.conditions.operator)) {
    _field.conditions.operator = "&&";
  }
  if (util.is_omitted_value(_field.conditions.javascript)) {
    _field.conditions.javascript = {};
  } else if (_.isString(_field.conditions.javascript)) {
    _field.conditions.javascript = {
      "default": _field.conditions.javascript
    };
  }
  if (util.is_omitted_value(_field.conditions.registered)) {
    _field.conditions.registered = {};
  } else if (_.isString(_field.conditions.registered)) {
    var name = _field.conditions.registered;
    _field.conditions.registered = {};
    _field.conditions.registered[name] = true;
  }
  return _field;
};
