...
var _validate_validations = function (_input, _field) {
  ...
  _.each(validations.javascript, function(value, key) {
    var result = _validate_javascript_validation(_input, _field, value);
    if (!result.valid) {
      results[key] = result.error;
    }
  });

  _.each(validations.registered, function(value, key) {
    var result = _validate_registered_validation(_input, key);
    if (!result.valid) {
      results[key] = result.error;
    }
  });

  var validators = Object.keys(validations.javascript).length + 
    Object.keys(validations.registered).length;
...
var _validate_conditions = function (_input, _field, _inputs) {
  ...
  _.each(conditions.javascript, function(value, key) {
    var result = _validate_javascript_condition(_input, _field, value);
    if (result.skipped) {
      results[key] = result.skipped;
    }
  });

  _.each(conditions.structured, function(value, key) {
    var result = _validate_structured_condition(key, value, _inputs);
    if (result.skipped) {
      results[key] = result.skipped;
    }
  });

  var validators = Object.keys(conditions.javascript).length + 
    Object.keys(conditions.structured).length;
