...
var _validate_validations = function (_input, _field) {
  ...
  var keys = Object.keys(validations.javascript);
  for (var i = 0, len = keys.length; i < len; ++i) {
    var result = _validate_javascript_validation(
      _input, validations.javascript[keys[i]]
    );
    if (!result.valid) {
      results[keys[i]] = result.error;
    }
  }

  // TODO check registered validations

  var errors = Object.keys(results).length;
  var valid = errors === 0 || 
    (validations.operator === "||" && errors < keys.length);
 ...
 */
var _validate_registered_validation = function (_input, _condition) {
