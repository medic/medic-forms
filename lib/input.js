...
var _ = require('underscore'),
  ...
  server_mod = require("./server_module.js");


/**
 * @name getEventEmitter:
 *   This code is used for unit testing, and should be moved.
 */
...
var _validate_validations = function (_input, _field) {
  ...
  var jsKeys = Object.keys(validations.javascript);
  for (var i = 0, len = jsKeys.length; i < len; ++i) {
    var result = _validate_javascript_validation(
      _input, _field, validations.javascript[jsKeys[i]]
    );
    if (!result.valid) {
      results[jsKeys[i]] = result.error;
    }
  }

  var registeredKeys = Object.keys(validations.registered);
  for (var i = 0, len = registeredKeys.length; i < len; ++i) {
    var result = _validate_registered_validation(
      _input, registeredKeys[i]
    );
    if (!result.valid) {
      results[registeredKeys[i]] = result.error;
    }
  }

  var validators = jsKeys.length + registeredKeys.length;
  var errors = Object.keys(results).length;
  var valid = errors === 0 || 
    (validations.operator === "||" && errors < validators);

  if (!valid) {
    return {
      valid: false,
      error: 'Input did not pass custom validation',
      detail: results
    }
  }
 ...
 */
var _validate_javascript_validation = function (_input, _field, _condition) {
  ...
  if (!util.exec('return ' + _condition, args, this).result) {
    return {
      valid: false,
      error: 'Input did not pass custom validation: ' + _condition
    }
  }
 ...
 */
var _validate_conditions = function (_input, _field, _inputs) {
  ...
  var jsKeys = Object.keys(conditions.javascript);
  for (var i = 0, len = jsKeys.length; i < len; ++i) {
    var result = _validate_javascript_condition(
      _input, _field, conditions.javascript[jsKeys[i]]
    );
    if (result.skipped) {
      results[jsKeys[i]] = result.skipped;
    }
  }

  var structuredKeys = Object.keys(conditions.structured);
  for (var i = 0, len = structuredKeys.length; i < len; ++i) {
    var result = _validate_structured_condition(
      structuredKeys[i], conditions.structured[structuredKeys[i]], _inputs
    );
    if (result.skipped) {
      results[structuredKeys[i]] = result.skipped;
    }
  }

  var validators = jsKeys.length + structuredKeys.length;
  var skips = Object.keys(results).length;
  var skip = skips === validators || 
    (conditions.operator === "||" && skips > 0);
  ...
  if (skip) {
    return { 
      ...
      skipped: true,
      detail: results,
      error: _input ? 'Skipped inputs must not have a value' : undefined
    };
  }
 ...
 *   the field referenced by the given `_condition_id` is equal to
 *   the given `_condition_value`.
 */
var _validate_structured_condition = function (_condition_id, _condition_value, _inputs) {
  return { 
    valid: true, 
    skipped: _inputs[_condition_id] === _condition_value 
  };
};
 ...
 */
exports.validate_all = function (_inputs, _fields) {
  ...
  for (var i = 0, len = _fields.length; i < len; ++i) {
    ...
    var input = _inputs[field.id];
    var r = exports.validate_any(input, field, _inputs);
 ...
 */
exports.validate_any = function (_input, _field, _inputs, _force_required) {
  ...
  var rv = _validate_conditions(_input, _field, _inputs);
...
exports.validate_image = function (_input, _field) {
  ...
  /* Are we running server-side? If not, we're done. */
  if (!server_mod || !server_mod.mmm || !_field.validationReady) {
    return { valid: true };
  }

  /* Server-side: File type detection library */
  var mmm = server_mod.mmm;
  var mmmagic = new server_mod.mmm.Magic(mmm.MAGIC_MIME_TYPE);
  ...
  mmmagic.detect(buf, function (err, result) {
    ...
    if (!exports.image_mime_supported[result]) {
      return emitter.emit(readyEvent, {
        valid: false,
        error: "Not a supported image type"
      });
    }

    emitter.emit(readyEvent, _validate_common(_input, _field));
  });
