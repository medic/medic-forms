...
var _validate_fieldset_identifiers = function (_map, _fields, _context) {
  ...
  for (var i = 0, len = _fields.length; i < len; ++i) {
    ...
    if (_map[id]) {
      return {
        valid: false, context: context, duplicate: id,
...
/**
 * @name validate_form_identifiers:
 *   One of several functions that performs additional form validation
 *   steps not currently possible using JSON schema. This function ensures
 *   that every form identifier used in the `_forms` array is unique. This
 *   function *should* be run after the schema validation step -- however,
 *   out of sheer paranoia, many of the schema-enforced constraints are
 *   double-checked here.
 */
exports.validate_form_identifiers = function (_forms) {
  ...
  for (var i = 0, len = _forms.length; i < len; ++i) {
    ...
    if (!_.isObject(form)) {
      return {
        valid: false,
        reason: 'Form ' + i + ' is not an object'
      };
    }
    ...
    if (!_.isObject(meta)) {
      return {
        valid: false,
        reason: 'Metadata for form ' + i + ' is missing or not an object'
      };
    }
    ...
    if (!_.isString(id) || id.length <= 0) {
      return {
        valid: false,
        reason: 'Form ' + i + ' has a missing or zero-length identifier'
      };
    }
    ...
    if (map[id] == id) {
      return {
        valid: false,
        reason: 'Form ' + i + ' has duplicate identifier `' + id + '`'
      };
    }
