...
var _rewrite_references = function (_subtree,
  ...
  if (_.isArray(_subtree)) {
    ...
    _.each(_subtree, function (_subitem, _i) {
      _rewrite_references(_subitem, _reference_root, _subtree, _i);
    });

  } else if (util.is_plain_object(_subtree)) {

    /* First run?
     *   We can't replace reference directives that occur at the top
     *   level, because we have no way of finding the object that
    ...
    if (!is_first_run && (_subtree.$ref || '').length > 0) {
      ...
      _.each(_subtree, function (_v, _k) {
        _rewrite_references(
          _subtree[_k], _reference_root, _subtree, _k
        );
      });
    }
  }
